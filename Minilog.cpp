...
namespace  //anonymous namespace: likes static global
{
    ...
    void format_timestamp(std::ostream & os, uint64_t timestamp)
    {
        ...
        auto gmtime = std::gmtime(&time_t);  //it gets GMT time instead of local time
        char buffer[32];
...
namespace minilog
{
    typedef std::tuple < char, uint32_t, uint64_t, int32_t, int64_t, double, MiniLogLine::string_literal_t, char * > SupportedTypes;
    ...
    void MiniLogLine::encode(Arg arg)
    {
    ...
    template < typename Arg >
    void MiniLogLine::encode(Arg arg, uint8_t type_id)
    {
    ...
    MiniLogLine::MiniLogLine(LogLevel level, char const * file, char const * function, uint32_t line)
            : m_bytes_used(0)
    ...
    void MiniLogLine::stringify(std::ostream & os)
    {
    ...
    template <>
    char * decode(std::ostream & os, char * b, MiniLogLine::string_literal_t * dummy)
    {
        MiniLogLine::string_literal_t s = *reinterpret_cast < MiniLogLine::string_literal_t * >(b);
        os << s.m_s;
        return b + sizeof(MiniLogLine::string_literal_t);
    }
    ...
    void MiniLogLine::stringify(std::ostream & os, char * start, char const * const end)
    {
        if (start == end)
            return;
    ...
    void MiniLogLine::encode(char const * arg)
    {
        if (arg != nullptr)
            encode_c_string(arg, strlen(arg));
    }

    void MiniLogLine::encode(char * arg)
    {
        if (arg != nullptr)
            encode_c_string(arg, strlen(arg));
    }

    void MiniLogLine::encode_c_string(char const * arg, size_t length)
    {
        if (length == 0)
            return;
    ...
    struct BufferBase
    {
        ...
        virtual void push(MiniLogLine && logline) = 0;
        virtual bool try_pop(MiniLogLine & logline) = 0;
    };
    ...
    {
    public:
        struct alignas(64) Item //alignas(x) sets memory alignment as x
        {
            Item()
            ...
            char written;
            char padding[256 - sizeof(std::atomic_flag) - sizeof(char) - sizeof(MiniLogLine)];
            MiniLogLine logline;
        };
        ...
        void push(MiniLogLine && logline) override
        {
        ...
        bool try_pop(MiniLogLine & logline) override
        {
    ...
    public:
        struct Item
        {
            Item(MiniLogLine && MiniLogLine) : logline(std::move(MiniLogLine)) {}
            char padding[256 - sizeof(MiniLogLine)];
            MiniLogLine logline;
        };
        ...
        bool push(MiniLogLine && logline, unsigned int const write_index)
        {
        ...
        bool try_pop(MiniLogLine & logline, unsigned int const read_index)
        {
    ...
    public:
        ...
        void write(MiniLogLine & logline)
        {
            auto pos = m_os->tellp();
    ...
    {
    public:
        Minilogger(NonGuaranteedLogger ngl, std::string const & log_directory, std::string const & log_file_name, uint32_t log_file_roll_size_mb)
                : m_state(State::INIT)
                ...
                , m_thread(&Minilogger::pop, this)
        {
            m_state.store(State::READY, std::memory_order_release);
        }

        Minilogger(GuaranteedLogger gl, std::string const & log_directory, std::string const & log_file_name, uint32_t log_file_roll_size_mb)
                : m_state(State::INIT)
        ...
        void pop()
        {
            ...
            MiniLogLine logline(LogLevel::INFO, nullptr, nullptr, 0);
    ...
    std::unique_ptr < Minilogger > minilogger;
    std::atomic < Minilogger * > atomic_minilogger;

    bool minilog::operator==(MiniLogLine & logline)
    {
        atomic_minilogger.load(std::memory_order_acquire)->add(std::move(logline));
        return true;
    }
    ...
    {
        minilogger.reset(new Minilogger(ngl, log_directory, log_file_name, log_file_roll_size_mb));
        atomic_minilogger.store(minilogger.get(), std::memory_order_seq_cst);
    }
    ...
    {
        minilogger.reset(new Minilogger(gl, log_directory, log_file_name, log_file_roll_size_mb));
        atomic_minilogger.store(minilogger.get(), std::memory_order_seq_cst);
    }
